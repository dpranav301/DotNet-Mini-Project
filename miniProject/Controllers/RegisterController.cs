    ...
    public class RegisterController : Controller
    {
        ...
        public ActionResult Login()
        {
            if (Request.Cookies["mycookie1"] != null && Request.Cookies["mycookie2"] != null)
            {
                ...
                HttpContext.Session.SetInt32("LoginId", Convert.ToInt32(Request.Cookies["mycookie3"]));
        ...
        public ActionResult Login(LoginViewModel reguser)
        {
            try
            {
                if (Register.GetLoginUser(reguser.LoginName, reguser.Password))
                {
                    ...
                    HttpContext.Session.SetInt32("LoginId", Register.GetLoginUserNo(reguser.LoginName, reguser.Password));

                    if (reguser.CheckBox)
                    {
                        Response.Cookies.Append("mycookie1", reguser.LoginName);
                        Response.Cookies.Append("mycookie2", reguser.Password);
                        Response.Cookies.Append("mycookie3", ""+Register.GetLoginUserNo(reguser.LoginName, reguser.Password));
                    }
        ...
        public ActionResult Edit()
        {
            if (HttpContext.Session.GetString("LoginName") != null)
            {
                int id = HttpContext.Session.GetInt32("LoginId")??0;
